...
createPerturbationIdeal(ZZ,ZZ) := Ideal => (a,b) -> (
    ...
    specm := map(R,R, {x_1,x_2,z+4,y_1,y_2,q_1..q_(a*b)});
    smat := specm outMatrix;
    spol := det(smat);

    specma := map(R,R, join({x_1,x_2,z,y_1,y_2}, new List from a*b:0));

    DF2 := spol - specma spol; --Here we take the difference of P_v and P_0
    K := unique entries (coefficients(DF2, Variables => {z,x_1,x_2}))_1_0;

    proj := map(ZZ/(nextPrime(10000))[y_1..y_4,x_1..x_(a*b)], R, {y_1, y_2, 0, y_3, y_4, x_1 .. x_(a*b)});

    return ideal(K / proj)
)
createPerturbationIdeal(ZZ,ZZ,ZZ) := Ideal => (a,b,c) -> (
